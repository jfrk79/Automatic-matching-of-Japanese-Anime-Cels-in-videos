...
image_folder = r"../file/frames_seg/video1"

# 加载特征文件
data = np.load(features_file)
features_array = data['features']
image_paths = data['paths']
image_name_to_features = {os.path.splitext(os.path.basename(path))[0]: features_array[i] for i, path in enumerate(image_paths)}

# 归一化特征向量
features_array_normalized = features_array / np.linalg.norm(features_array, axis=1, keepdims=True)

# 使用FAISS构建余弦相似度索引（通过内积实现）
d = features_array.shape[1]
index = faiss.IndexFlatIP(d)
index.add(features_array_normalized)

# 创建字典以便快速查找图像文件路径
image_name_to_path = {os.path.basename(path): path for path in image_paths}

# 处理模拟照片
simulated_photos_path = r'../file/cels'
output_results_path = r'../file/withoutbackground'
